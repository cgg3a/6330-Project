...
int main (int argc, char *argv[])
{
    ...
    for (int i = 0; i < num_rows; i++){
        ...
        for (int j = 0; j < num_cols; j++){
            temp.push_back(rand() % 100 + 1); // Random number 1-100
        }
        data.push_back(temp);
    }
    ...
    for (int i = 0; i < num_threads - 1; i++){
        args[i + 1] = {(i + 1), num_threads, &data, num_rows, num_cols, &found, target, &foundMutex};
        pthread_create(&threads[i], NULL, searchPart, static_cast<void*>(&args[i + 1]));
    }
    // Set this thread to work, too
    args[0] = {0, num_threads, &data, num_rows, num_cols, &found, target, &foundMutex};
